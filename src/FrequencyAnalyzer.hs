...
-- Count the number of occurences of each alphabet character in the text. Excludes space.
textCharCounts :: [Char] -> String -> [Int]
textCharCounts alphabet text = map (`count` text) alphabet

-- Order characters in alphabet based on frequency in text. Excludes space.
